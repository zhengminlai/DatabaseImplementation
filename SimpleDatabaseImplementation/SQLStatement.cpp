...
void SQLSelect::Parse(vector<string> sql_vector)
{
	...
	else
	{
		while (boost::algorithm::to_lower_copy(sql_vector[pos]) != "from")
		{
			...
			if (sql_vector[pos] == "count" || sql_vector[pos] == "max" || sql_vector[pos] == "min" || sql_vector[pos] == "avg")
			{
				select_attribute_.push_back(sql_vector[pos] + sql_vector[pos + 1] + sql_vector[pos + 2] + sql_vector[pos + 3]);
				pos += 3;
			}
			else
				select_attribute_.push_back(sql_vector[pos]);
	...
	if (boost::algorithm::to_lower_copy(sql_vector[pos]) != "where")
		throw SyntaxErrorException();/*select * from student where*/
	pos++;

	while (true)
	{
		SQLWhere sql_where;
		sql_where.data_type = -1;//代表类型为非varchar或char类型
								 //table.attribute
		...
		int k = -1;
		bool f = true;
		if ((k = sql_vector[pos].find('.')) == string::npos)
			f = false;
		//若是table.attr
		if (f)
		{
			sql_where.key_2 = sql_vector[pos];
			pos++;
		}
		//值
		else
		{
			...
			//若是字符串
			if (sql_vector[pos][0] == '\'' || sql_vector[pos][0] == '\"')
			{
				//取'' 或 ""里的内容
				sql_where.data_type = T_CHAR;
		...
		if (boost::algorithm::to_lower_copy(sql_vector[pos]) != "and")
...
void SQLJoinSelect::Parse(vector<string> sql_vector)
{
	...
	for (; pos <= from_pos; pos++)
	{
		...
		{/*
		 ...
		 throw SyntaxErrorException();*/
			if (sql_vector[pos - 1] == ")")
				selected_info_.push_back(sql_vector[pos - 4] + sql_vector[pos - 3] + sql_vector[pos - 2] + sql_vector[pos - 1]);
			else
	...
	{//判断table.attr中的table是否合法
		...
		for (auto iterator = selected_info_.begin(); iterator < selected_info_.end(); iterator++)
		{
			...
			int Pos_ = tbName.find('.');
			tbName.assign(tbName.substr(0, Pos_));
			bool tbFlag = false;
	...
	{
		SQLWhere sql_where;
		sql_where.data_type = -1;//默认table.att=value中value是非字符串类型
...
TKey* TKey::operator/=(const TKey t1) {
	...
	{
	case 0:
		*(int*)key_ /= *(int*)(*temp).key_; break;
	case 1:
		*(float*)key_ /= *(float*)(*temp).key_; break;
	case 2:
		key_ = "";
		return nullptr;
	default:
